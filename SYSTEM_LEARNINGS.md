...
## 2026-09-13 — Swing horizons on daily bars (BTC and MSTR): first pass (summary; full table re-generated by `scripts/swing_target.py`)

Evaluation 2021-01 → 2026-08, tune = first half (bear/chop), validation = second half (strong bull:
BTC buy & hold +104 % over 5-day cycles, MSTR +118 % to +213 %). Hit rates on non-overlapping cycles.

**BTC.** No configuration beats momentum (sign of 63-day return) *and* buy & hold on the validation
half. Best validation hit 53–56 % came from configs that were at 37–50 % on tuning. Long/short
strategies lost 35–85 % against +104 % buy & hold because they shorted a bull market. For swing
BTC, holding beats predicting; the free on-chain / macro frame did not change that.

**MSTR (long / cash only).** Long-biased rules on an asset that tripled mostly look good by
construction: 10 of 12 configs are positive on validation but only 2 beat buy & hold, and they are
different configs at different horizons (5-day price-only LightGBM: tune 52.1 % hit, +107 % vs −1 %
B&H; validation 53.9 %, +234 % vs +118 %, Sharpe 1.04 — the only one positive on *both* halves;
21-day on-chain logistic: validation 64.7 % on ~34 cycles = noise-level sample). One lead
(MSTR 5-day, price features) is worth a robustness check (seeds, thresholds, drawdown, longer
history); nothing here is a validated edge yet.

**Learning.** At swing horizons the dominant factor is the regime (bear vs bull), not the model.
A model that cannot beat "always long" in a bull half and "always cash" in a bear half is
regime-timing, and regime timing is what MVRV / stable-coin flows are supposed to give — which
they did not, here, on 2.5-year halves. Next: robustness of the MSTR 5-day lead; a regime
classifier (bull / bear from MVRV + 200-day trend) as a *position-size* input for buy & hold rather
than a direction predictor.
